...
<head>
    ...
    <title>{{ isset($product) ? 'Edit' : 'Add' }} Service - Setepak Printing</title>
...
<body>
    <div class="form-container">
        <h2>{{ isset($product) ? 'Edit' : 'Add New' }} Printing Service</h2>

        <form method="POST" action="{{ isset($product) ? route('vendor3.products.update', $product->id) : route('vendor3.products.store') }}">
            @csrf
            @if(isset($product))
                @method('PUT')
            @endif
            ...
            <div style="display: flex; gap: 1rem; margin-top: 2rem;">
                <button type="submit" class="btn btn-primary">
                    ...
                    {{ isset($product) ? 'Update' : 'Create' }} Service
                </button>
