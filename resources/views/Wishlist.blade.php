                ...
                @foreach($wishlist as $id => $item)
                    <div class="wishlist-card">
                        @php
                            ...
                            // Check if product has a thumb_image first
                            if (isset($item['thumb_image'])) {
                                $imagePath = asset($item['thumb_image']);
                            }
                            // If no thumb_image, check vendor_id if available
                            else if (isset($item['vendor_id'])) {
                                if ($item['vendor_id'] == 3) {
                                    // Richiamo Caffe products
                                    $imageName = str_replace(' ', '-', $productName) . '.png';
                                    $imagePath = asset('images/caffe/' . $imageName);
                                } elseif ($item['vendor_id'] == 2) {
                                    // Vendor3 (Setepak Printing Service) products - use FontAwesome icons
                                    $imagePath = 'icon'; // Special flag for icon-based products
                                } else {
                                    // UTM Mart products
                                    // Extract the actual product ID from composite key or use as is
                                    $actualProductId = isset($item['product_id']) ? $item['product_id'] : (strpos($id, '-') !== false ? explode('-', $id)[1] : $id);

                                    // Try to find product in the database first
                                    $product = \App\Models\Product::find($actualProductId);
                                    if ($product && $product->thumb_image) {
                                        $imagePath = asset($product->thumb_image);
                                    }
                                    // If not found, fallback to keyword-based logic
                                    else if (str_contains($productName, 'shirt')) {
                                        $imagePath = asset('images/utmmart/utm-shirt.jpeg');
                                    } elseif (str_contains($productName, 'cup')) {
                                        $imagePath = asset('images/utmmart/utm-cup.jpeg');
                                    } elseif (str_contains($productName, 'book')) {
                                        $imagePath = asset('images/utmmart/utm-books.jpeg');
                                    } else {
                                        $imagePath = asset('images/utmmart/utm-shirt.jpeg');
                                    }
                                }
                        ...
                        @else
                            <img src="{{ $imagePath }}" alt="{{ $item['name'] }}"
                                 onerror="this.src='{{ asset('images/utm-shirt.jpeg') }}'">
                        @endif
                        <h4>{{ $item['name'] }}</h4>
